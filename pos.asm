...
;---checkout
	...
	CHECKOUT_PRICEATAX DB 0, 0, 0, 0 ;PRICE AFTER TAX
	CHECKOUT_PRICEBTAX DB 0, 0, 0, 0 ;PRICE BEFORE TAX
...
PRINT_RECEIPT PROC NEAR
	...
	;---------display
			...
			XOR AX,AX
			XOR DX,DX
			ADD CHECKOUT_PRICEATAX(0), 30H
			ADD CHECKOUT_PRICEATAX(1), 30H
			ADD CHECKOUT_PRICEATAX(2), 30H
			MOV AH, 02H
			MOV DL, CHECKOUT_PRICEATAX(0)
			INT 21H
			MOV AH, 02H
...
CAL_TAX PROC NEAR
			XOR AX, AX				; COMBINE DIGIT1 & DIGIT 2
			MOV AL, CHECKOUT_TOT(0) 
			MUL TEN
			ADD AL, CHECKOUT_TOT(1)
			MUL TEN
			ADD AL, CHECKOUT_TOT(2)

			MUL TAX
			DIV HUN
			DIV HUN 
			MOV CHECKOUT_PRICEATAX(0),AL 
			MOV DL, AH
			XOR AX,AX
			MOV AL,DL
			DIV HUN
			MOV CHECKOUT_PRICEATAX(1),AL
			MOV DL, AH
			XOR AX,AX
			MOV AL,DL
			DIV TEN
			MOV CHECKOUT_PRICEATAX(2),AL
			MOV CHECKOUT_PRICEATAX(3),AL
			RET
CAL_TAX ENDP
