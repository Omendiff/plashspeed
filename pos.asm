...
;------LOGIN 
  	LOGIN_M1 DB 10, 13, 10, 13, "-------PLASHSPEED POS-------"
  			 DB 10, 13, " ENTER USERNAME: $"
	LOGIN_M2 DB 10, 13, " ENTER PASSWORD: $"
	LOGIN_M3 DB 10, 13, "-----ACCESS GRANTED-----$"
	LOGIN_M4 DB 10, 13, "-----ACCESS DENIED------"
			 DB 10, 13, " PLEASE LOGIN AGAIN!!!$"
	LOGIN_M5 DB 10, 13, 10, 13, " ALPHABETS ONLY!!!$"
...
;---inventory menu
	INV_MENU DB 10, 13, 10, 13, "---------INVENTORY MANAGEMENT---------"
			 ...
			 DB 10, 13, "CHOICE: $"
	;----SET REMINDER
	INV_M1 DB 10, 13, 10, 13, "MIN NUMBER OF PRODUCT IN STORE: $"
...
;---staff change password
	STF_M1 DB 10, 13, 10, 13, "----CHANGE ADMIN PASSWORD---- "
...
;---ADMIN STAFF
	STF_M17 DB 10, 13, "------STAFF MANAGEMENT------"
			DB 10, 13, " ENTER ADMIN PASSOWRD: $"
...
;---return and refund
	...
	RTN_M2 DB 10, 13, 10, 13, "TOTAL REFUND AMOUNT(INCLUDING TAX): RM $"
	RTN_M3 DB 10, 13, "RETURN SUCCEED, PROCEED TO REFUND.... $"
	RTN_M4 DB 10, 13, "-------RETURN & REFUND-------"
		   DB 10, 13, " ENTER RECEIPT ID: $"
	RTN_M5 DB 10, 13, " INCORRECT RECEIPT ID!!! $"
	RTN_M6 DB 10, 13, " RECEIPT ID: $"
	RTN_M7 DB 10, 13, 10, 13, " APPROVE REFUND??? "
		   DB 10, 13, " 1. YES "
		   DB 10, 13, " 2. NO "
		   DB 10, 13, 10, 13, " CHOICE: $"
	RTN_M8 DB 10, 13, 10, 13, "----REFUND RECEIPT--- "
		   DB 10, 13, "RECEIPT ID: A0001R "
		    DB 10, 13, "ITEM     QUANTITY    PRICE(RM)$"
	RTN_M9  DB 10, 13, " SHIRT      $"
	RTN_M10 DB 10, 13, " SHORTS     $"
	RTN_M11 DB 10, 13, " HATS       $"
	RTN_M12 DB 10, 13, " SOCKS      $"
	RTN_M13 DB "         $"
	RTN_M DB 10, 13, " $"
	RECEIPT_NO DB "A0001$"
	RECEIPT_ID DB 6 
...
;---REGISTER NEW MEMBER
	...
	IC_ADD DB 12 DUP(0)
	PHONE_ADD DB 10 DUP(0)
	IC_REMOVE DB 12 DUP(0)
	PHONE_REMOVE DB 10 DUP(0)
...
;---SALES REPORT
	SALES_M1  DB 10, 13, "PLEASE ENTER DATE: $"
	SALES_M2  DB 10, 13, "PLEASE ENTER PRODUCT SOLD: $"
	SALES_M3  DB 10, 13, "PLEASE ENTER TOTAL AMOUNT OF THE PRODUCT SOLD: $"
	SALES_M4  DB 10, 13, "THE TOTAL NET PROFIT WILL BE RM $"
	SALES_SHIRT DB 0,0
	SALES_HATS DB 0,0
	SALES_SHORTS DB 0,0
	SALES_SOCKS DB 0,0
	PROFIT DB 0,0,0
	SALES_M6 DB 10, 13, 10, 13, "----SALES REPORT--- "
		     DB 10, 13, "REPORT ID: RP0001 "
		     DB 10, 13, "ITEM     QUANTITY    PROFIT(RM)$"
	SALES_SHIRT2 DB 0
	SALES_HATS2 DB 0
	SALES_SHORTS2 DB 0
	SALES_SOCKS2 DB 0
...
LOGIN:
	...
	CHECKDIGIT:
		...
		LOWER:               ;CHECK IF LESS THAN 30 OR NOT
        	CMP SI, 3
			JE LOGIN_PASS    
...
PRINT_REFUND_AMOUNT PROC NEAR
	PRINT RTN_M8
	REFUND_SHIRT:
		CMP CHECKOUT_PRICEBTAX_SHIRT, 0
		JE REFUND_SHORTS
		...
		INT 21H
	REFUND_SHORTS:
		CMP CHECKOUT_PRICEBTAX_SHORTS, 0
		JE REFUND_HATS

		PRINT RTN_M10
		MOV AH, 02H
		MOV DL, CHECKOUT_QTY2
		INT 21H
		PRINT RTN_M13
		XOR AX, AX
		MOV AL, CHECKOUT_PRICEBTAX_SHORTS
		DIV HUN
		MOV RTN_SHORTS[0], AL
		MOV DL, AH
		XOR AX, AX
		...
		MOV RTN_SHORTS[1], AL
		MOV RTN_SHORTS[2], AH
		ADD RTN_SHORTS[0], 30H
		ADD RTN_SHORTS[1], 30H
		ADD RTN_SHORTS[2], 30H
		MOV AH, 02H
		MOV DL, RTN_SHORTS[0]
		INT 21H
		MOV AH, 02H
		...
		INT 21H
	REFUND_HATS:
		CMP CHECKOUT_PRICEBTAX_HATS, 0
		JE REFUND_SOCKS

		PRINT RTN_M11
		MOV AH, 02H
		MOV DL, CHECKOUT_QTY3
		INT 21H
		PRINT RTN_M13
		XOR AX, AX
		MOV AL, CHECKOUT_PRICEBTAX_HATS
		DIV HUN
		MOV RTN_HATS[0], AL
		MOV DL, AH
		XOR AX, AX
		...
		INT 21H
	REFUND_SOCKS:
		CMP CHECKOUT_PRICEBTAX_SOCKS, 0
		JE REFUND_TOTAL

		PRINT RTN_M12
		MOV AH, 02H
		MOV DL, CHECKOUT_QTY4
		INT 21H
		PRINT RTN_M13
		XOR AX, AX
		MOV AL, CHECKOUT_PRICEBTAX_SOCKS
		DIV HUN
		MOV RTN_SOCKS[0], AL
		MOV DL, AH
		XOR AX, AX
...
ADD_CUSTOMER PROC NEAR                              
   ;-----2. PROMPT FOR IC
	...
	IC_CHECKDIGIT:
			   ...
			   IC_LOWER:               ;CHECK IF LESS THAN 12 OR NOT
			         CMP SI, 12
					 JE PHONE_INPUT   
	...
	PHONE_INPUT:
			PRINT MEMBER_M2            
			;PART 2:READ USER INPUT
			MOV CX, 10            ;SET COUNTER TO READ 4 BYTES
			MOV SI, 0             ;SAVE DATA INTO PW
	...
	INPUTPHONE:
		...
		PHONE_CHECKDIGIT:
			   ...
			   PHONE_LOWER:               ;CHECK IF LESS THAN 10 OR NOT
			         CMP SI, 10
					 JE MEMBER_ADD   
					 
					 CMP PHONE_ADD[SI], 30H   
					 JL PHONE_ERROR2      ;NOT DIGIT 
					 JGE PHONE_UPPER
			   PHONE_UPPER:               ;CHECK IF MORE THAN 39
			         CMP PHONE_ADD[SI], 39H
					 JG  PHONE_ERROR2     ;NOT DIGIT
					 JLE [PHONE_NEXTP]
...
MEMBER_REMOVE PROC NEAR
	...
	RMV_IC_CHECKDIGIT:
			   ...
			   RMV_IC_LOWER:               ;CHECK IF LESS THAN 12 OR NOT
			         CMP SI, 12
					 JE REMOVE_PHONE
					 
					 CMP IC_REMOVE[SI], 30H   
					 JL IC_RMV_ERROR2      ;NOT DIGIT 
					 JGE RMV_IC_UPPER
			   RMV_IC_UPPER:               ;CHECK IF MORE THAN 39
			         CMP IC_REMOVE[SI], 39H
					 JG  IC_RMV_ERROR2     ;NOT DIGIT
					 JLE [IC_RMV_NEXTP]
	...
	RMV_PHONE:
			...
			INT 21H
		  	...
		  	MOV PHONE_REMOVE[SI], AL     
		   	INC SI
		   	LOOP RMV_PHONE

		RMV_PHONE_CHECKDIGIT:
	           MOV SI, 0
			   XOR AX, AX
			   
			   RMV_PHONE_LOWER:               ;CHECK IF LESS THAN 10 OR NOT
			         CMP SI, 10
					 JE MEMBER_REMOVE1  
					 
					 CMP PHONE_REMOVE[SI], 30H   
					 JL RMV_PHONE_ERROR2      ;NOT DIGIT 
					 JGE RMV_PHONE_UPPER
			   RMV_PHONE_UPPER:               ;CHECK IF MORE THAN 39
			         CMP PHONE_REMOVE[SI], 39H
					 JG  RMV_PHONE_ERROR2     ;NOT DIGIT
					 JLE [RMV_PHONE_NEXTP]
...
PRINT_PROFIT PROC NEAR
	...
	PROFIT_SHIRT:
		...
		MOV SALES_SHIRT2,AL
        DIV TEN
		MOV SALES_SHIRT[0], AL
		MOV SALES_SHIRT[1], AH
		ADD SALES_SHIRT[0], 30H
		ADD SALES_SHIRT[1], 30H
		MOV AH, 02H
		MOV DL, SALES_SHIRT[0]
		INT 21H
		MOV AH, 02H
		MOV DL, SALES_SHIRT[1]
		INT 21H
		...
		MOV AL, CHECKOUT_QTY2
        MUL ORI_SHORTS
		MOV SALES_SHORTS2,AL
        DIV TEN
		MOV SALES_SHORTS[0], AL
		MOV SALES_SHORTS[1], AH
		ADD SALES_SHORTS[0], 30H
		ADD SALES_SHORTS[1], 30H
		MOV AH, 02H
		MOV DL, SALES_SHORTS[0]
		INT 21H
		MOV AH, 02H
		MOV DL, SALES_SHORTS[1]
		INT 21H
		...
		MOV SALES_HATS[0], AL
		MOV SALES_HATS[1], AH
		ADD SALES_HATS[0], 30H
		ADD SALES_HATS[1], 30H
		MOV AH, 02H
		MOV DL, SALES_HATS[0]
		INT 21H
		MOV AH, 02H
		...
		MOV SALES_SOCKS2,AL
        DIV TEN
		MOV SALES_SOCKS[0], AL
		MOV SALES_SOCKS[1], AH
		ADD SALES_SOCKS[0], 30H
		ADD SALES_SOCKS[1], 30H
		MOV AH, 02H
		MOV DL, SALES_SOCKS[0]
		INT 21H
		MOV AH, 02H
		MOV DL, SALES_SOCKS[1]
		INT 21H

		PROFIT_TOTAL:
		XOR AX,AX
		MOV AL,SALES_SHIRT2
		ADD AL,SALES_SHORTS2
		ADD AL,SALES_HATS2
		ADD AL,SALES_SOCKS2
		DIV HUN
		MOV PROFIT[0],AL
